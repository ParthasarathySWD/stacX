...
class MyOrders extends MY_Controller {
	...
	public function index()
	{
		...
		$data['permission'] = $this->Common_Model->GetRolePermissions($this->RoleUID);
		...
		$data['is_vendorlogin'] = $is_vendorlogin;
		...
		if($is_vendorlogin){
			...
			$data['groupsbyloggedid']  = $this->MyOrders_Model->get_vendor_groups($is_vendorlogin,$vendoruids,$this->loggedid);

			if(!empty($vendors)){

				$data['lastviewed_orders'] = $this->MyOrders_Model->vendor_lastviewed_orders($this->loggedid,$vendors[0]->VendorUID);
			}else{
				$data['lastviewed_orders'] = (object)[];
			}

		}else{

			$data['groupsbyloggedid'] = $this->MyOrders_Model->get_groupsby_loggedid();
			$data['lastviewed_orders'] = $this->MyOrders_Model->lastviewed_orders($this->loggedid);
		...
		foreach ($data['lastviewed_orders'] as $key => $value) {

			$assignedusers =  $this->MyOrders_Model->get_order_assigned_users($value,$Workflows,$is_vendorlogin);

			if(!empty($assignedusers)){
				$data['lastviewed_orders'][$key]['LoginID'] = $assignedusers;
			}else{
				$data['lastviewed_orders'][$key]['LoginID'] = '';
			}
			...
			if (in_array($this->session->userdata('RoleType'),array(1,2,3,4,5,6,13)) == False){
				...
				if($assigned_orderss === array_intersect($assigned_orderss, $completed_orderss) && $completed_orderss === array_intersect($completed_orderss, $assigned_orderss)) {
					...
					if($assigned_workflows === array_intersect($assigned_workflows, $completed_workflows) && $completed_workflows === array_intersect($completed_workflows, $assigned_workflows)) {
						unset($data['lastviewed_orders'][$key]);
					} 
		...
		$Customer_Check = $this->MyOrders_Model->GetCustomerByUserUID($this->loggedid)->CustomerUID;
		$CustomTablevalues = $this->MyOrders_Model->GetCustomTablevalues($this->loggedid)[0]->COUNT;


		/*Custom sort check*/
		if($CustomTablevalues >= 1 )
		{

			if($Customer_Check == 0){

				$data['content'] = 'index';
				$data['Action'] = 'CustomTables';
				$data['THeadContent'] = $this->MyOrders_Model->GetTHeadContent($this->loggedid);
				$data['TBodyContent'] = [];
				$CustomFieldName = $this->MyOrders_Model->GetCustomFieldNameByUserUID($this->loggedid);

				$FieldName =[];

				foreach ($CustomFieldName as $childArray) {
					$FieldName[] = $childArray['FieldName'];
				}

				$DynTable = implode($FieldName, ',');
				$data['Sort'] = $this->MyOrders_Model->GetOrderByField($this->loggedid);
				$Sort = $data['Sort']->FieldSortBy;

				if($is_vendorlogin){

					if(!empty($vendors)){

						$checkorders = $this->MyOrders_Model->GetTBodyContent($this->loggedid,$DynTable,$Sort,$vendors[0]->VendorUID);
					...
					$checkorders = $this->MyOrders_Model->GetTBodyContent($this->loggedid,$DynTable,$Sort);
				}
				...
				if(!empty($checkorders)){
					$data['TBodyContent']  = $checkorders;
				}

			}else{
				$data['content'] = 'customer_order';
			} 
	...
	{ 
		...
		if($is_vendorlogin){
			...
			if(!empty($vendors)){
				...
				$checkorders = $this->MyOrders_Model->get_vendor_orders($this->loggedid,$post,$vendors[0]->VendorUID);
			}

		}else{

			$checkorders = $this->MyOrders_Model->check_order($this->loggedid,$post);
		}
		...
		if(!empty($checkorders))
		{
			if($post['length']!='') 
			{
				$my_orders = array_slice($checkorders, $post['start'],$post['length']); // Ajax Pagination Limit get array
			} else {
				$my_orders = array();
			}
		} else {
		...
		$data = array();      
		$TATOrdersUIDs = GetTotalTATOrders();	

		foreach ($my_orders as $orders) {
			$row =  $this->myorder_datatable($orders, $is_vendorlogin, $logged_details,$TATOrdersUIDs);
			$data[] = $row;
		}
		$data = array(
			'data' => $data,
		...
		if($is_vendorlogin){
			...
			$vendors  = $this->MyOrders_Model->get_vendors($logged_details,$this->loggedid);
			if(!empty($vendors)){
				$vendortotalorders=$this->MyOrders_Model->get_vendor_countall_orders($this->loggedid,$post,$vendors[0]->VendorUID,$Workflows,$is_vendorlogin);
				$output = array(
					"draw" => $post['draw'],
					"recordsTotal" => $vendortotalorders,
					"recordsFiltered" => $vendortotalorders,
					"data" => $data['data'],
				);
				if ($post['search_value']!='' && isset($post['search_value'])) {
					$output['recordsFiltered']=$this->MyOrders_Model->get_vendor_countfiltered_orders($this->loggedid,$post,$vendors[0]->VendorUID,$Workflows,$is_vendorlogin);
				}
			}

		}else{
			$totalrecords=$this->MyOrders_Model->count_check_order($this->loggedid,$post);
			$output = array(
				"draw" => $post['draw'],
